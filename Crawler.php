...
class Crawler extends CrawlerBase
{
    ...
    public function crawl($con, $incremental, $cached)
    {
        ...
        if ($con == 'all') {
            ...
            foreach ($info as $key => $value) {
                if ($incremental && !empty($this->problemModel->basic($this->problemModel->pid('UVaLive' . $key)))) {
                    continue;
                }
        ...
        if ($failures) {
            ...
            $len = 22;
            $last = 'UVaLive' . $failures[count($failures) - 1];
    ...
    private function _crawl($con, $info)
    {
        ...
        $this->line("<fg=yellow>{$updmsg}:   </>UVaLive$con");
        ...
        $this->pro['pcode'] = 'UVaLive' . $con;
        $this->pro['OJ'] = $this->oid;
        $this->pro['contest_id'] = null;
        $this->pro['index_id'] = $info[0];
        $this->pro['origin'] = "https://icpcarchive.ecs.baylor.edu/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=" . $info[0];
        ...
        $this->pro['description'] = "<a href=\"/external/gym/UVaL{$con}.pdf\">[Attachment Link]</a>";
        $this->pro['input'] = '';
        $this->pro['output'] = '';
        $this->pro['note'] = '';
        $this->pro['sample'] = [];
        ...
        $this->line("<fg=green>$donemsg:    </>UVaLive$con");
        return true;
    }
}
